...
#         input_feat[..., 8:] = build_color_volume(rays_pts, pose_ref, imgs, img_feat, with_mask=True, downscale=img_downscale)
        ...
        # only append the rgb values of the nearest view
        input_feat[..., 8:] = build_color_volume(rays_pts, pose_ref, imgs[:, :3], img_feat, with_mask=True, downscale=img_downscale)
    else:
        input_feat = index_point_feature(volume_feature, rays_ndc) if torch.is_tensor(volume_feature) else volume_feature(rays_ndc)
    return input_feat

def rendering(args, pose_ref, rays_pts, rays_ndc, depth_candidates, rays_o, rays_dir, records,
    ...
    records['1_sample'].append(start_1.elapsed_time(end_1))
    ##############################################################
    ...
    end_2.record()
    torch.cuda.synchronize()
    records['2_nerf'].append(start_2.elapsed_time(end_2))
    ##############################################################
    ...
    end_3.record()
    torch.cuda.synchronize()
    records['3_rend'].append(start_3.elapsed_time(end_3))
    ##############################################################
    
    ret = {}

    return rgb_map, input_feat, weights, depth_map, alpha, ret, records
